...
async def debug_ml_test(
    ...
    """Endpoint temporário para testar respostas da ML API."""
    ...
    if not account or not account.access_token:
        return {"error": "No active ML account"}

    results = {}
    async with MLClient(account.access_token) as client:
        # Get first item
        items_resp = await client.get_user_listings(account.ml_user_id, limit=2)
        item_ids = items_resp.get("results", [])
        results["item_ids"] = item_ids[:2]

        if item_ids:
            # Raw item data
            item = await client.get_item(item_ids[0])
            results["item_raw"] = {
            ...
            # Test visits - time_window
            try:
                visits_tw = await client._request(
                    "GET",
                    f"/items/{item_ids[0]}/visits/time_window",
                    params={"last": 3, "unit": "day"},
                )
                results["visits_time_window"] = visits_tw
            except Exception as e:
                results["visits_time_window_error"] = str(e)

            # Test visits - bulk
            today = date_type.today().isoformat()
            try:
                visits_bulk = await client._request(
                    "GET",
                    "/visits/items",
                    params={"ids": item_ids[0], "date_from": today, "date_to": today},
                )
                results["visits_bulk_raw"] = visits_bulk
            except Exception as e:
                results["visits_bulk_error"] = str(e)

            # Test orders
            try:
                orders = await client._request(
                    "GET",
                    "/orders/search",
                    params={
                ...
                results["orders_count"] = orders.get("paging", {}).get("total", 0)
                results["orders_sample"] = [
                    {
                        ...
                        "items": [
                            {"mlb": oi.get("item", {}).get("id"), "qty": oi.get("quantity")}
                            for oi in o.get("order_items", [])
                        ],
                    }
                    for o in orders.get("results", [])[:3]
                ]
            except Exception as e:
